...
from models.transformer_custom.transformer_custom import *

def train_detr_dataloader(save_filename,logging_filename,signals_path, train_dataset_list,val_dataset_list,load_FeatureExtract_filename,batch_size = 10000,
    ...
    # custom dataloader train version
    train_dataset = Sleep_Dataset_sequence(data_path=signals_path,dataset_list=train_dataset_list,class_num=5,
    use_scaling=use_scaling,scaling=scaling,use_noise=False,epsilon=epsilon,noise_scale=noise_scale,
    ...
    train_dataloader = DataLoader(dataset=train_dataset,batch_size=2, shuffle=True, num_workers=16)
    val_dataloader = DataLoader(dataset=val_dataset,batch_size=1, shuffle=False, num_workers=16)
    
    # train_dataset.length , val_dataset.length
    ...
    # featureExtract.module.load_state_dict(torch.load(load_FeatureExtract_filename))


    # model = DETR_psg(num_classes=5, hidden_dim=256, nheads=8,
    ...
    ENC_DROPOUT = 0.1
    DEC_DROPOUT = 0.1
    print('info : ', INPUT_DIM, HIDDEN_DIM, ENC_LAYERS, ENC_HEADS, ENC_PF_DIM, ENC_DROPOUT, device)
    ...
    if torch.cuda.device_count() > 1:
        ...
        # featureExtract = nn.DataParallel(featureExtract)
    ...
    for epoch in range(epochs):
        if epoch == 0:
            ...
            for index,data in enumerate(train_dataloader):
                ...
                # [ batch_size , sequence_length , channel , sample_rate * epoch_size] dim = 4
                batch_signal,batch_label = data

                # [ batch_size * sequence_length , channel , sample_rate * epoch_size] dim = 3
...
def training_detr_dataloader():
    model_save_path = '/home/eslab/kdy/git/Hallym_SleepStage/saved_model/Hallym_dataset/transformer/'
    logging_save_path = '/home/eslab/kdy/git/Hallym_SleepStage/log/Hallym_dataset/transformer/'

    # Feature Extract로 활용한 모델 pre-trained file path
    ...
    epsilon = 0.8
    noise_scale = 2e-6
    scheduler = 'WarmUp_restart_gamma' # 'WarmUp_restart'

    load_FeatureExtract_filename = model_load_path + 'ResNet_ensemble_branch_dataloaer_hallym_%.4f_%s_%s_%s.txt'%(learning_rate,optim,loss_function,scheduler)
    save_filename = model_save_path + 'ResNet_ensemble_branch_detr_dataloaer_hallym_%.4f_%s_%s_%s.txt'%(learning_rate,optim,loss_function,scheduler)
    logging_filename = logging_save_path + 'ResNet_ensemble_branch_detr_dataloaer_hallym_%.4f_%s_%s_%s.pth'%(learning_rate,optim,loss_function,scheduler)

    train_detr_dataloader(save_filename=save_filename,logging_filename=logging_filename,signals_path=signals_path, train_dataset_list=training_fold_list,val_dataset_list=validation_fold_list,
                                                load_FeatureExtract_filename=load_FeatureExtract_filename,epochs=epochs,batch_size=batch_size,learning_rate=learning_rate,
                                        optim=optim,loss_function=loss_function,epsilon=epsilon,noise_scale=noise_scale,
                                        use_noise=use_noise,preprocessing=preprocessing,preprocessing_methods=preprocessing_methods,scheduler=scheduler,warmup_iter=10,cosine_decay_iter=40,stop_iter=300
                                        )
