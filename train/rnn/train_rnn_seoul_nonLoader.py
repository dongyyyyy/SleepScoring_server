...
def train_rnn_dataloader_onePerson_seoul_Loader(save_filename,logging_filename,load_filename,signals_path, train_dataset_list,val_dataset_list,batch_size = 10000,
                                          ...
                                          sequence_length=0,use_cudaNum=0,len_divide=10,window_size=1,num_layers=1,hidden_dim=512):
    ...
    Classification = lstm_onePerson(flat=512*5,num_layers=num_layers, hidden_dim=hidden_dim)
...
def training_rnn_dataloader_onePerson_seoul_Loader(use_cudaNum=1,sequence_length=0,window_size=25,num_layers=1,hidden_dim=512):
    ...
    for scheduler in scheduler_list:
        ...
        save_filename = model_save_path + 'ResNet_ensemble_branch_dataloaer_seoul_3channel_0.0100_AdamW_CE_WarmUp_restart_gamma_eachPerson_lstm_Loader.pth'
        logging_filename = logging_save_path + 'ResNet_ensemble_branch_dataloaer_seoul_3channel_0.0100_AdamW_CE_WarmUp_restart_gamma_eachPerson_lstm_Loader.txt'

        train_rnn_dataloader_onePerson_seoul_Loader(save_filename=save_filename,logging_filename=logging_filename,load_filename=load_filename,signals_path=signals_path, train_dataset_list=training_fold_list,val_dataset_list=validation_fold_list,
                                                    epochs=epochs,batch_size=batch_size,learning_rate=learning_rate,
                                            optim=optim,loss_function=loss_function,epsilon=epsilon,noise_scale=noise_scale,
                                            use_noise=use_noise,preprocessing=preprocessing,preprocessing_methods=preprocessing_methods,scheduler=scheduler,warmup_iter=5,cosine_decay_iter=15,stop_iter=50,sequence_length=sequence_length,use_cudaNum=use_cudaNum,
